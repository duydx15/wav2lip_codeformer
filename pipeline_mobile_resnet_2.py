...
def detection_face(mobile_net,resnet_net, img, device,padding_ratio = None):
	H,W,_ = img.shape
	...
	for k in range(dets.shape[0]):
		...
		face_idx.append(score)

	truth_face_coordinate = []
	truth_score = 0
	highest_ycenter_bottomright = 0
	index_truth_face = 0
	# print("Length:", len(l_coordinate),'-', len(face_idx))
	for index, coordinate in enumerate(l_coordinate):
		...
		if y_center > highest_ycenter_bottomright:
			...
			index_truth_face = int(np.floor(index/2))
			truth_face_coordinate = [(scale_topleft, scale_bottomright)]



	return truth_face_coordinate, detected_face, face_idx[index_truth_face]

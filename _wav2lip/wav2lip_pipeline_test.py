...
def merge_color(videopts_out,videopts,videopts_beard,videopts_beard_inner,frame,box,pred,alpha,facemesh):
    ...
    # if not landmark_gan is None or not face_landmarks_gan is None:
...
def lipsync_one_frame(frame, mel_chunk, facemesh, MASK_KP_IDS,  wav2lip_model, gfpgan_model,mobile_net, resnet_net, device,to_tensor,model_Occ):
    ...
    frame_tmp2 = frame.copy()
    frame_tmp2[y1:y2, x1:x2, :] = cv2.GaussianBlur(roi, (15,15), 0)
    frame_tmp[y1:y2, x1:x2, :] = pred
    video_h,video_w = frame.shape[:2]
    mask_mount =  np.zeros((video_h,video_w), np.uint8)
    cv2.fillPoly(mask_mount,pts = [videopts_out], color=(255,255,255))
    mask_lips =  np.zeros((video_h,video_w), np.uint8)
    cv2.fillPoly(mask_lips,pts = [videopts_lips], color=(255,255,255))
    intersect_mask = mask_mount - mask_lips
    intersect_mask = cv2.GaussianBlur(intersect_mask, (17,17), 0)
    frame_tmp2 = blend_images_using_mask(frame_tmp2,frame_tmp,intersect_mask)
    pred = frame_tmp2[y1:y2, x1:x2, :]
    ...
    frame = merge_color(videopts_out,videopts,videopts_beard,videopts_beard_inner,frame,box,restored_img,0.95,facemesh)
...
def main():
    ...
    savepath = 'Wav2lip_8Sep_Dr_ES.mp4'
    ...
    vqfr_modelpath  = '/home/ubuntu/Duy_test_folder/Retinaface_Mediapipe/VQFR/experiments/pretrained_models/VQFR_v1-33a1fac5.pth'

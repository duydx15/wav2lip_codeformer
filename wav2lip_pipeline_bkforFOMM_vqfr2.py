...
from load_vqfr import load_vqfr_model
...
def gen_lipsync_img(img, mel_chunk, model, box, device):
    ...
    # if version == '1.0':
    # _,_,pred = vqfr_model.enhance(pred, has_aligned=False, only_center_face=False, paste_back=True)
    # if version == '2.0':
    #     _,_,pred = vqfr_model.enhance(pred,fidelity_ratio=1, has_aligned=False, only_center_face=False, paste_back=True)
...
def get_box_by_RetinaFace(inimg, mobile_net_wav2lip, resnet_net_wav2lip, device):
    ...
    for r_idx in range(len(padding_ratio)):
        l_coordinate, detected_face= detection_face_wav2lips(mobile_net_wav2lip,resnet_net_wav2lip, inimg, device,padding_ratio[r_idx])
...
def merge_color(videopts_out,videopts,videopts_beard,videopts_beard_inner,frame,box,pred,alpha,facemesh):
    ...
    # cv2.polylines(output_main, [videopts_out],True,color=(0,255,0),thickness=2)
...
def lipsync_one_frame(frame, frame_ori,mel_chunk, facemesh, MASK_KP_IDS,  wav2lip_model,\
                        mobile_net_wav2lip, resnet_net_wav2lip, device,vqfr_model):
    """
    """
    # globaloldFrame,pts
    # box = face_detect([frame], sfd_facedetector)[0]
    ...
    box = get_box_by_RetinaFace(frame, mobile_net_wav2lip, resnet_net_wav2lip, device)
    if box==None:
        print("Not detect")
        return frame_ori
    ...
    # print(type(pred))
    # _,_,restored_img = vqfr_model.enhance(pred, has_aligned=False, only_center_face=False, paste_back=True)
    restored_img = pred
    # frame_tmp = merge_color(videopts_out,videopts,videopts_beard,videopts_beard_inner,frame_ori,box,restored_img,0.95,facemesh)
    # # cv2.rectangle(frame,(x1,y1),(x2,y2),(0,255,0),2)
    # restored_img = frame_tmp[y1:y2,x1:x2, :]
    _,_,restored_img = vqfr_model.enhance(restored_img, has_aligned=False, only_center_face=False, paste_back=True)

    frame = merge_color(videopts_out,videopts,videopts_beard,videopts_beard_inner,frame_ori,box,restored_img,0.95,facemesh)
    # frame[y1:y2, x1:x2, :] = pred#restored_img
...
if __name__=='__main__':
    # vidpath = 'Dr_es_Wav2lipGAN_DFL_audio.mp4'#_wav2lip/DrDisrespect-Falls-in-Love-with-Warzone-again-thanks-to-new-Game-Mode-30FPS.mp4'
    # frame_path = 'Dr_video/DrDisrespect-Falls-in-Love-with-Warzone-again-thanks-to-new-Game-Mode-30FPS.mp4'
    # savepath = 'Wav2lipGan_fullmouth_VQFR_debug_lipsyncx2_p2.mp4'
    # wavpath = '_wav2lip/Dr_es_0p17.wav'

    vidpath = '/home/ubuntu/Duy_test_folder/Retinaface_Mediapipe/DFLive_test/Penguinz_spanish/charlietest.mp4'#_wav2lip/DrDisrespect-Falls-in-Love-with-Warzone-again-thanks-to-new-Game-Mode-30FPS.mp4'
    frame_path = '/home/ubuntu/Duy_test_folder/Retinaface_Mediapipe/DFLive_test/Penguinz_spanish/charlietest.mp4'
    #'Dr_video/DrDisrespect-Falls-in-Love-with-Warzone-again-thanks-to-new-Game-Mode-30FPS.mp4'
    savepath = '/home/ubuntu/Duy_test_folder/Retinaface_Mediapipe/DFLive_test/Penguinz_spanish/Wav2lipgan_charlietest_lipsync_vqfr.mp4'
    wavpath = '/home/ubuntu/Duy_test_folder/Retinaface_Mediapipe/DFLive_test/Penguinz_spanish/charlietest_lipsync.wav'
    ...
    # frame_start = 500
    # frame_stop = 1200
    total_output_frames = frame_stop #16092# min(total_frames, len(mel_chunks))
    count_frame = 0
    ...
    for i in tqdm(range(total_output_frames)):
        ...
        # elif count_frame == 10023:
            ...
            res = lipsync_one_frame(frame,frame_ori, mel_chunk, facemesh, MASK_KP_IDS,\
              wav2lip_model, mobile_net_wav2lip, resnet_net_wav2lip, device,vqfr_model)
